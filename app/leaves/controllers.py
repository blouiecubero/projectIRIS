...
def leaves():
	...
	file_owner = User.query.filter_by(username=whosuser).first()

	# if user has applied for leave
	if request.method == 'POST' and request.form['submitButton']:
	...
	# Parse the data. Assumed that the dates end with semicolon
		for i in addDate:
	...
	elif request.method == 'POST' and request.form['chooseUser']:
		print "hi"
